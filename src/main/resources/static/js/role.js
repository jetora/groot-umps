...
// 点击查询按钮
function searchData() {
    $('#role_table').bootstrapTable('refreshOptions', {pageNumber: 1, offset: 0});
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#role_table').bootstrapTable({
            url: '/api/role/pageInfo',                      //请求后台的URL（*）
            method: 'GET',                      //请求方式（*）
            dataType: "json",
            ...
            onLoadError: function (data) {
                ...
                //console.log(data);
                //$('#role_table').boostrapTable('load', savedData);
            },
...
function addData() {
    ...
    // 初始化form表单内容
    roleModal.find('.modal-body #opt_type').val("add");
    roleModal.find('.modal-body #id').val("");
    roleModal.find('.modal-body #rolename').val("");
    roleModal.find('.modal-body #description').val("");
    roleModal.find('.modal-body #enabled').selectpicker('val',1);
    roleModal.modal("show");
}
function editData(row) {
    $('#modalTitle').text('修改角色');
    var roleModal = $('#roleModal');
    roleModal.find('.modal-body #opt_type').val("edit");
    roleModal.find('.modal-body #id').val(row.id);
    roleModal.find('.modal-body #rolename').val(row.name);
    roleModal.find('.modal-body #description').val(row.description);
...
function get_form_data() {
    ...
    var roleModal = $('#roleModal');
    // 获取表单内容
    data.opt_type = roleModal.find('.modal-body #opt_type').val();
    data.id = roleModal.find('.modal-body #id').val();
    data.rolename = roleModal.find('.modal-body #rolename').val();
    data.description = roleModal.find('.modal-body #description').val();
